...
int process_redir(t_redir *temp_redir, t_parse *temp, t_mini *prog)
{
    ...
    if (temp_redir->type == REDIR_IN && handle_redir_in(temp_redir, temp, prog) < 0)
        return (-1);
    if (temp_redir->type == REDIR_OUT && handle_redir_out(temp_redir->filename, temp, prog) < 0)
        return (-1);
    if (temp_redir->type == REDIR_APPEND && handle_redir_append(temp_redir->filename, temp, prog) < 0)
        return (-1);
    return (0);  
}
